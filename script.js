...
const translations = {
  ...
  ro: {
    ...
    heroTitle: "Programează o consultație cu Forest & Co",
    heroText:
    ...
    paymentNote: "Alege tipul consultației, apoi finalizează plata în siguranță prin Stripe.",
    hilexQuestion: "Ești membru HiLex?",
    yes: "Da",
    no: "Nu",
    noConsultationFee: "Fără taxă de consultație",
    consultationFeeApplies: "Se aplică taxa de consultație",
    appointmentType: "Tipul programării",
    online: "Online",
    inPerson: "În persoană",
    consultationLength: "Durata consultației",
...
function updateSummary() {
  ...
  if (!selectedDate) {
    appointmentSummary.textContent =
      currentLanguage === "ro"
        ? `${t("chooseWeekday")} pentru o consultație de ${duration} minute (${feeLabel}).`
...
function showReceipt(booking, options = {}) {
  ...
  receiptText.textContent =
    options.message ||
    (currentLanguage === "ro"
      ? `${booking.fullName}, consultația ta pentru ${(booking.areaOfLaw || booking.caseType).toLowerCase()} este programată pentru ${formatDate(new Date(`${booking.date}T00:00:00`))} ${t("at")} ${booking.time}. ${t("confirmationEmailSent")} ${booking.email}.`
